...
ThorFlow 是胸外科围术期流程协同平台：医生可以看清每位患者当前进度，患者可以收到下一步提醒，系统将检查、手术、病理和随访连成完整流程。
...
ThorFlow 将这些环节整理成一条清晰的数字化流程：医生端像调度台一样查看全局，患者端像小程序一样告知下一步操作，中间由 Workflow Engine 生成任务、通知和随访计划。
...
1. 打开 `/dashboard`：查看医生端全局流程池
2. 打开 `/patient-mini`：查看患者当前阶段和医生通知
3. 打开 `/patient-mini/tasks`：体验上传检查单和 AI 识别检查项目
4. 打开 `/dashboard/preop`：查看医生端收到术前检查完成提醒
5. 打开 `/patient/PT-1004`：查看患者详情、timeline 和关键操作
6. 打开 `/dashboard/pathology`：体验录入病理和分期
7. 打开 `/patient-mini/followup`：查看患者端随访计划
